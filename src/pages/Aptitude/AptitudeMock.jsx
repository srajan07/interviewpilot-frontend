...
function AptitudeMock() {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      {/* Header Bar */}
      <div className="bg-white rounded-xl border border-[#E5E7EB] p-3 sm:p-5"
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <h1 className="text-base sm:text-lg font-bold text-[#111827]">Aptitude Mock Test</h1>
          </div>
          <div className="flex items-center gap-2">
            <div className={`flex items-center gap-1.5 px-3 py-2 rounded-lg border font-mono font-bold text-sm ${
              timeLeft < 300 ? "border-[#FCA5A5] bg-[#FEE2E2] text-[#DC2626]" : "border-[#E5E7EB] bg-[#F5F5F0] text-[#111827]"
            }`}>
              <span className="text-[10px] font-normal font-sans text-[#9CA3AF] hidden sm:inline">Remaining</span>
              {formatTime(timeLeft)}
            </div>
      ...
      <div className="flex flex-col-reverse lg:flex-row gap-6">
        {/* Question Panel */}
        <div className="flex-1 bg-white rounded-xl border border-[#E5E7EB] p-5 sm:p-7 space-y-6"
            ...
            {currentQ.options.map((option, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-3.5 sm:p-4 rounded-xl border transition-all flex items-center gap-3 ${
                    isSelected
                ...
                >
                  ...
                  <span className={`text-sm min-w-0 ${isSelected ? "text-[#1E3A5F] font-semibold" : "text-[#374151]"}`}>
                    {option}
                  </span>
                </button>
              );
        ...
        {/* Question Palette */}
        <div className="bg-white rounded-xl border border-[#E5E7EB] p-4 sm:p-5 space-y-4 lg:w-52 lg:shrink-0 h-fit"
          style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
          ...
          <div className="pt-3 border-t border-[#F3F4F6] space-y-2 text-[11px] text-[#9CA3AF]">
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded bg-[#DBEAFE] border border-[#BFDBFE] shrink-0"></span>
              <span>Answered ({answeredCount})</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded bg-[#F5F5F0] border border-[#E5E7EB] shrink-0"></span>
